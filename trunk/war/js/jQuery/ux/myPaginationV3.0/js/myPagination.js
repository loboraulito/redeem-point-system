/*!
 * myPagination Jquery Pagination Plug-in Library v3.0
 * 
...
(function($){
	...
	function init(param,obj){
		if(param&&param instanceof Object){
			var options;var currPage;var pageCount;var pageSize;var tempPage;
			var defaults=new Object({
				currPage:1,pageCount:10,pageSize:5,cssStyle:'badoo',
				ajax:{on:false,hiddenId:'pageCount',param:{on:false,page:1},
					ajaxStart:function(){return false}
				},
				info:{
					first:'首页',last:'尾页',next:'下一页',prev:'上一页',
					first_on:true,last_on:true,next_on:true,prev_on:true,
					msg_on:true,link:'#',
			...
			function getMsg(){
				...
				if(options.info&&options.info.msg){
					var str=options.info.msg;str=str.replace("{curr}",input);
					str=str.replace("{sum}",pageCount);
					return str
				}else{
					var str=defaults.info.msg;str=str.replace("{curr}",input);
					str=str.replace("{sum}",pageCount);
					return str
				}
			...
			function updateView(){
				...
				if(firstPage+pageSize>pageCount){
					...
					//firstPage=lastPage-pageSize// 这样写将会导致出现负数页
				}else{
				...
				content+=getFirst();
				if(currPage==1){
					content+="<span class=\"disabled\" title=\""+getPrev()+"\" style='cursor:hand'>"+getPrev()+" </span>"
				}else{
					content+="<a href='"+link+"' title='"+(currPage-1)+"' style='cursor:hand'>"+getPrev()+" </a>"
				}
				for(firstPage;firstPage<lastPage;firstPage++){
					if(firstPage==currPage){
						content+="<span class=\"current\" title=\""+firstPage+"\" style='cursor:hand'>"+firstPage+"</span>"
					}else{
						content+="<a href='"+link+"' title='"+firstPage+"' style='cursor:hand'>"+firstPage+"</a>"
					}
				}
				if(currPage==pageCount){
					content+="<span class=\"disabled\" title=\""+getNext()+"\" style='cursor:hand'>"+getNext()+" </span>"
				}else{
					content+="<a href='"+link+"' title='"+(currPage+1)+"' style='cursor:hand'>"+getNext()+" </a>"
				}
				...
				if(ajax.on){
					...
					$.ajax({
						...
						success:function(data){
							var option = {};
							option.data = data;
							option.dataType = ajax.dataType;
							option.callback = ajax.callback;
							loadPagionData(option);
							/*
							load2PageCount({
							...
							*/
							...
							if(ajax.dataType){
								...
								default:
									isB=false;
									var callback=ajax.callback+"(data)";
									eval(callback);
			...
			function loadPagionData(options){
				if(options.dataType){
			...
			/*
			function load2PageCount(options){
